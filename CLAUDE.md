...
- **段階駆動スキル**（上表・core）: discover / specify / plan / tasks / implement。SDDのワークフローを前進させる手続き役。discover（発見）→ specify（清書）の順で、生成を急がず前段で詰める。
...
- .claude/agents/ … 役割別サブエージェント。**ワーカー役**（product-analyst・quality-analyst・solution-analyst・scenario-author、および active プロファイル提供の backend／frontend-engineer）が各段階で Skill を呼んで実作業し、**レビュー役**（spec-compliance・test-coverage・code-reviewer）が独立コンテキストで検証する。
